...
def qsub_prep(script_path, cpus, mem, smiles_idx, run_nr, smiles, s_factor,
              time_ps, k_push, alp, random_seed, method, with_products=False):
    """
    ...
    """.format(script_path, cpus, mem, smiles_idx, run_nr, smiles, s_factor,
               time_ps, k_push, alp, pwd, random_seed, method, with_products)
...
def run_calculations(smiles_df, n_runs, script_path, s_factor, time_ps, k_push,
                     alp, max_queue, cpus, mem, random_seeds, method,
                     with_products=False):
    """
    ...
    for j, idx in enumerate(smiles_df.index):
        ...
        for run_nr in range(n_runs):
            qsub_name = qsub_prep(script_path, cpus, mem, idx, run_nr, smiles,
                                  ...
                                  with_products=with_products, method=method)
            if with_products:
            ...
            if len(submitted_jobs) >= max_queue:
                while True:
                    job_info = os.popen("squeue -u dpg158").readlines()[1:]
...
if __name__ == "__main__":
    ...
    SCRIPT = '/groups/kemi/dpg158/PHD/metaMD/metaMD.py'

    _DIR = str(S_FACTOR)+'_'+str(K_PUSH)+'_'+str(ALP)+'_'+str(METHOD)
